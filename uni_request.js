export default function ({ baseURL, timeout = 12345, header: headers, statusCode = [200, 401] }) {
	return {
		get(url, data, header) { return this.request('GET', url, data, { ...header, ...headers }) },
		post(url, data, header) { return this.request('POST', url, data, { ...header, ...headers }) },
		put(url, data, header) { return this.request('PUT', url, data, { ...header, ...headers }) },
		delete(url, data, header) { return this.request('DELETE', url, data, { ...header, ...headers }) },
		connect(url, data, header) { return this.request('CONNECT', url, data, { ...header, ...headers }) },
		head(url, data, header) { return this.request('HEAD', url, data, { ...header, ...headers }) },
		options(url, data, header) { return this.request('OPTIONS', url, data, { ...header, ...headers }) },
		reace(url, data, header) { return this.request('TRACE', url, data, { ...header, ...headers }) },
		uploadFile(url, data, header) { return this.file('uploadFile', url, data || {}, { ...header, ...headers }) },
		downloadFile(url, data, header) { return this.file('downloadFile', url, data || {}, { ...header, ...headers }) },
		onerror: [], // 请求错误钩子函数集合
		file(method, url, data, header) {
			...
			return new Proxy(new Promise((resolve, reject) => { // 返回经过 Proxy 后的 Promise 对象使其可以监听到是否调用 abort 和 onProgressUpdate 方法
				this.interceptors.request.intercept({ header: header || {}, body: data.formData || {} }, method, url, data).then(async ({ header, body }) => { // 等待请求拦截器里的方法执行完
					...
					requestTask = uni[method]({
					    ...
					    success: async res => {
							...
							this.interceptors.response.intercept(statusCode.includes(res.statusCode) ? resolve : reject,  { success: statusCode.includes(res.statusCode), ...res }, method, url, data, reject) // 执行响应拦截器
					    },
						fail: async res => {
							clearTimeout(timer)
